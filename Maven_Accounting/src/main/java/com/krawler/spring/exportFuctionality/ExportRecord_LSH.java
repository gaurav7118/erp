...
public class ExportRecord_LSH extends ExportRecordBeans implements MessageSourceAware{
...
//                }
                ...
                if(mode == StaticValues.AUTONUM_BILLINGSALESORDER||mode==StaticValues.AUTONUM_SALESORDER){
                    ...
                    theader = messageSource.getMessage("acc.field.custquotationversion", null, RequestContextUtils.getLocale(request));
                    datetheader = theader;
                }else if(mode == StaticValues.AUTONUM_VENQUOTATION){
...
//                tab5.addCell(cell3);
                ...
                if(mode==StaticValues.AUTONUM_INVOICE){
                    ...
                    shipAddr=CommonFunctions.getBillingShippingAddress(sOrder.getBillingShippingAddresses(),false);
                    if(StringUtil.isNullOrEmpty(shipAddr)){ //For old record in which addresses were saved in invoice table 
                    ...
                    billAddr = CommonFunctions.getBillingShippingAddress(sOrder.getBillingShippingAddresses(), true);//true used for billing address and false used for shipping address
                    if (StringUtil.isNullOrEmpty(billAddr)) { //For old record in which addresses were saved in invoice table 
                   ...
                   if (storageHandlerImpl.GetBCHLCompanyId().contains(com.getCompanyID()) && mode == StaticValues.AUTONUM_PURCHASEORDER) {
                    ...
                    shipAddr=CommonFunctions.getBillingShippingAddress(pOrder.getBillingShippingAddresses(),false);
                    if(StringUtil.isNullOrEmpty(shipAddr)){ //For old record in which addresses were saved in invoice table 
                        shipAddr=pOrder.getShipTo()==null?"":pOrder.getShipTo();
                    } 
                    
                    billtoAddress = CommonFunctions.getBillingShippingAddress(pOrder.getBillingShippingAddresses(), true);//true used for billing address and false used for shipping address
                    ...
                    shipAddr=CommonFunctions.getBillingShippingAddress(quotation.getBillingShippingAddresses(),false);
                    ...
                    billTo="Bill To";
                    isInclude=false;
                    billAddr = CommonFunctions.getBillingShippingAddress(quotation.getBillingShippingAddresses(), true);//true used for billing address and false used for shipping address
                    ...
                    customerEmail= quotationVersion.getCustomer()!=null?quotation.getCustomer().getEmail():"";
                    shipAddr=CommonFunctions.getBillingShippingAddress(quotationVersion.getBillingShippingAddresses(),false);
                    if(StringUtil.isNullOrEmpty(shipAddr)){ //For old record in which addresses were saved in quotation table 
                        shipAddr=quotation.getShipTo()==null?"":quotationVersion.getShipTo();
                    } 
                    terms=quotationVersion.getCustomer()!=null?quotationVersion.getCustomer().getCreditTerm().getTermname():"";
                    billTo="Bill To";
                    isInclude=false;
                    billAddr = CommonFunctions.getBillingShippingAddress(quotationVersion.getBillingShippingAddresses(), true);//true used for billing address and false used for shipping address
                    if (StringUtil.isNullOrEmpty(billAddr)) { //For old record in which addresses were saved in quotation table 
                    ...
                    addrParams.put("isBillingAddress", true);
                    billAddr= accountingHandlerDAOobj.getVendorAddress(addrParams);//for supplier billaddr will be vendor address
                    shipAddr = CommonFunctions.getBillingShippingAddress(venquotation.getBillingShippingAddresses(), false);
                    ...
                    billtoAddress = CommonFunctions.getBillingShippingAddress(venquotation.getBillingShippingAddresses(), true);//true used for billing address and false used for shipping address
...
//                mainTable.addCell(mainCell14);
                ...
                boolean companyFlag=(storageHandlerImpl.GetVRnetCompanyId().contains(com.getCompanyID())&&(mode==StaticValues.AUTONUM_INVOICE||mode==StaticValues.AUTONUM_QUOTATION||mode==StaticValues.AUTONUM_CUSTOMERQUOTATIONVERSION||mode==StaticValues.AUTONUM_SALESORDER));
...
//                            prodDesc += "\n\n Part No. :";
                        ...
                        if(!StringUtil.isNullOrEmpty(row10.getDescription())){
                            prodDesc = row10.getDescription();
                        }else{
                           if (!StringUtil.isNullOrEmpty(row7.getProduct().getDescription())) {
                                prodDesc = row10.getProduct().getDescription();
                            }
                        }
                        ...
                        uom = row10.getProduct().getUnitOfMeasure()==null?"":row10.getProduct().getUnitOfMeasure().getNameEmptyforNA();
                    }else if(mode==StaticValues.AUTONUM_VENQUOTATION){
...
//                   invcell = createCell(prodDesc, fontSmallRegular, Element.ALIGN_LEFT, Rectangle.LEFT + Rectangle.RIGHT, 5);
                 ...
                 if(!addShipTo){   
                   ...
                   // if (mode == StaticValues.AUTONUM_INVOICE || mode==StaticValues.AUTONUM_SALESORDER || mode==StaticValues.AUTONUM_PURCHASEORDER || mode==StaticValues.AUTONUM_QUOTATION) {
                    ...
                    table.addCell(invcell);
                    if(mode!=StaticValues.AUTONUM_RFQ) {
                        if( mode==StaticValues.AUTONUM_PURCHASEORDER || mode==StaticValues.AUTONUM_SALESORDER || mode==StaticValues.AUTONUM_CUSTOMERQUOTATIONVERSION ||mode==StaticValues.AUTONUM_QUOTATION || mode==StaticValues.AUTONUM_VENQUOTATION){
                            bAmt = accCurrencyobj.getBaseToCurrencyAmount(requestParams, rate, currencyid, entryDate, 0);
...
//                tab5.addCell(cell3);
                ...
                if(mode==StaticValues.AUTONUM_GOODSRECEIPT){
                    ...
                    if (storageHandlerImpl.GetVRnetCompanyId().contains(com.getCompanyID())) {
                        ...
                        billAddress = CommonFunctions.getBillingShippingAddress(gr.getBillingShippingAddresses(), true);//true used for billing address and false used for shipping address
                    ...
                    shipAddress=CommonFunctions.getBillingShippingAddress(gr.getBillingShippingAddresses(),false);
...
//                com = mode != StaticValues.AUTONUM_BILLINGRECEIPT ? rc.getCompany() : rc1.getCompany();
                ...
                if(mode == StaticValues.AUTONUM_BILLINGRECEIPT) {
                    ...
                    paidTo=rc1.getReceivedFrom()!=null?rc1.getReceivedFrom().getValue():"";
...
//                    journalEntryDate = rc.getJournalEntry().getEntryDate();
                    ...
                    paidTo=rc.getReceivedFrom()!=null?rc.getReceivedFrom().getValue():"";
...
//                    journalEntryDate = pc.getJournalEntry().getEntryDate();
                    ...
                    paidTo=pc.getPaidTo()!=null?pc.getPaidTo().getValue():"";
                    memo = pc.getMemo();
                    com = pc.getCompany();
...
//                }
                ...
                ExportRecordHandler.addHeaderContents(CompanyPDFHeader,document, writer);
                ExportRecordHandler.addHeaderFooter(CompanyPDFFooter,document, writer); 
                ExportRecordHandler.generateReceiptLSHPDF(authHandlerDAOObj, kwlCommonTablesDAOObj, EnglishNumberToWordsOjb, 
                        messageSource, mainTable,request, com, logoPath, currencyid, receiptNumber, 
                        journalEntryDate, formatter, accname, address, amount, mode, PayDetail,paidTo, memo, 
                    ...
                    if(mode == StaticValues.AUTONUM_DELIVERYORDER){
                        ...
                        billTo="Supplier";
                        billAddres = grOrder.getVendor().getAddress();
...
//                    tab5.addCell(cell3);
		        ...
		        linkHeader="VI/PO No.";
                        ...
                        String[] headerDetails = {"DO. No.", "Status", "Due Date", "Ship Date", "Ship Via", "FOB"};
                        String referenceNumber=ExportRecordHandler.getReferenceNumber(idresult.getEntityList(),mode);
                        ...
                        String[] headerDetails = {"GR. No.", "Status", "Due Date", "Ship Date", "Ship Via", "FOB"};
                        String referenceNumber=ExportRecordHandler.getReferenceNumber(idresult.getEntityList(),mode);
                        PdfPTable detailsTable=ExportRecordHandler.getDetailsTable(headerDetails,referenceNumber,status,null,shipDate,formatter,true, shipvia, fob);
                        mainCell12 = new PdfPCell(detailsTable);
                    }                    
