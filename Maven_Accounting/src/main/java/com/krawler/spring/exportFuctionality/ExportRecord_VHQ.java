...
public class ExportRecord_VHQ  extends ExportRecordBeans implements MessageSourceAware{
    ...
    public ByteArrayOutputStream createVHQPdf(HttpServletRequest request, String currencyid, String billid, DateFormat formatter, int mode, double amount, String logoPath, String customer, String accname, String address,boolean isExpenseInv,String CompanyID,String userId, String baseCurrency) throws DocumentException, ServiceException, IOException {
        ...
        try {
            ...
            writer.setPageEvent(new ExportRecord_VHQ.EndPage());
            document.open();
...
//                    entryDate = (creNote.isNormalCN()) ? creNote.getJournalEntry().getEntryDate() : creNote.getCreationDate();
                    ...
                    fieldrequestParams.put(Constants.filter_values, Arrays.asList(com.getCompanyID(), Constants.Acc_Credit_Note_ModuleId, 0, 1));
                    ...
                    Detailfilter_params.add(com.getCompanyID());
                    Detailfilter_names.add("journalentryId");
                    Detailfilter_params.add(creNote.getJournalEntry().getID());
                    ...
                    if (idcustresult.getEntityList().size() > 0) {
                        ...
                        for (Map.Entry<String, Object> varEntry : variableMap.entrySet()) {
                            ...
                            if (!StringUtil.isNullOrEmpty(coldata)) {
                                if ((varEntry.getKey().split("_")[1]).equals("Job Number")) {
                                    jobNumber = coldata;
                                }
                            }
                        }
...
//                    entryDate = (dbNote.isNormalDN())?dbNote.getJournalEntry().getEntryDate():dbNote.getCreationDate();
                    ...
                    HashMap<String, Object> fieldrequestParams = new HashMap();
                    fieldrequestParams.put(Constants.filter_names, Arrays.asList(Constants.companyid, Constants.moduleid, Constants.customcolumn, "customfield"));
                    fieldrequestParams.put(Constants.filter_values, Arrays.asList(com.getCompanyID(), Constants.Acc_Debit_Note_ModuleId, 0, 1));
                    FieldMap = accAccountDAOobj.getFieldParamsCustomMap(fieldrequestParams, replaceFieldMap, customFieldMap, customDateFieldMap);
                    ...
                    Detailfilter_params.add(com.getCompanyID());
                    Detailfilter_names.add("journalentryId");
                    Detailfilter_params.add(dbNote.getJournalEntry().getID());
                    Detailfilter_names.add("moduleId");
                    Detailfilter_params.add(moduleid + "");
                    invDetailRequestParams.put("filter_names", Detailfilter_names);
                    invDetailRequestParams.put("filter_params", Detailfilter_params);
...
//                Company com = mode != StaticValues.AUTONUM_BILLINGINVOICE ? inv.getCompany() : inv1.getCompany();
                ...
                PdfPTable table1 = new PdfPTable(2);
                table1.setWidthPercentage(100);
                table1.setWidths(new float[]{60, 40});

                tab1 = ExportRecordHandler.addCompanyTemplateLogoForVQH(logoPath, com);
                tab2 = new PdfPTable(1);
                PdfPCell invCell = null;
...
//                mainTable.addCell(mainCell11);
                ...
                PdfPTable userTable2 = new PdfPTable(2);
                userTable2.setWidthPercentage(100);
                userTable2.setWidths(new float[]{75, 25});

//                tab3 = ExportRecordHandler.getCompanyInfo(company);

                PdfPTable tab4= ExportRecordHandler.getDateTableWithJobIDforVHQ(entryDate,invno,numHeader,formatter,invCell,jobNumber);

                cell1 = new PdfPCell(tab1);
                cell1.setBorder(0);
                userTable2.addCell(cell1);
                cel2 = new PdfPCell(tab4);
                cel2.setBorder(0);
...
//                tab5.addCell(cell3);
                ...
                if(mode==StaticValues.AUTONUM_BILLINGCREDITNOTE){
                    ...
                    if (customerObj == null) {
                        customerName = creNote.getVendor().getName();
                        customerEmail =  creNote.getVendor() != null ?  creNote.getVendor().getEmail() : "";
//                        addrParams.put("vendorid", creNote.getVendor().getID());
//                        addrParams.put("companyid", creNote.getCompany().getCompanyID());
//                        addrParams.put("isBillingAddress", false);
//                        shipAddress = accountingHandlerDAOobj.getVendorAddress(addrParams);
                        terms = customerObj != null ? creNote.getVendor().getDebitTerm().getTermname() : "";
                    } else {
                        customerName = customerObj.getName();//inv.getCustomer()==null?inv.getCustomerEntry().getAccount().getName():inv.getCustomer().getName();
                        customerEmail = customerObj != null ? customerObj.getEmail() : "";
//                        addrParams.put("customerid", customerObj.getID());
...
//                    billAddress = accountingHandlerDAOobj.getCustomerAddress(addrParams);
                    ...
                    if (vendorObj == null) {
                        customerName = dbNote.getCustomer().getName();//dbNote.getCustomer().getName();
                        customerEmail = dbNote.getCustomer() != null ? dbNote.getCustomer().getEmail() : "";
                        terms = dbNote.getCustomer() != null ? dbNote.getCustomer().getCreditTerm().getTermname() : "";
//                        addrParams.put("customerid", dbNote.getCustomer().getID());
//                        addrParams.put("companyid", dbNote.getCompany().getCompanyID());                        
//                        addrParams.put("isBillingAddress", true);
//                        billAddress = accountingHandlerDAOobj.getCustomerAddress(addrParams);
                    } else {
                        customerName = vendorObj.getName();//dbNote.getCustomer().getName();
                ...
                while (itr.hasNext()) {
                    ...
                    if (mode == StaticValues.AUTONUM_CREDITNOTE ) {
                        ...
                        if(!otherwiseFlag){    
                            ...
                            cndnDiscount=row.getDiscount()!=null?row.getDiscount().getDiscount():0;
                        ...
                        try{
                            ...
                            cndnDiscount=row1.getDiscount()!=null?row1.getDiscount().getDiscount():0;
                            cndnTotalOtherwiseAmount+=cndnDiscount;
                        }
...
//                table.addCell(grcell); 
               ...
               // pcell1.setPaddingRight(0);
                ...
                if (mode == StaticValues.AUTONUM_RECEIPT) {
                    ...
                    if (rc.getCreatedby() != null) {
                    preparedBy = StringUtil.getFullName(rc.getCreatedby());
                    }
...
//                    config = new com.krawler.utils.json.base.JSONObject(rc1.getTemplateid().getConfigstr());
                   ...
                   if (inv.getCreatedby() != null) {
                    preparedBy = StringUtil.getFullName(inv.getCreatedby());
                    }
                    ...
                    preparedBy = StringUtil.getFullName(pc.getCreatedby());
                    }
//                    if (iscontraentryflag) {
//                        rc = (Receipt) kwlCommonTablesDAOObj.getClassObject(Receipt.class.getName(), billid);
//                    }
...
//                        String tempName = URLEncoder.encode((String) , "UTF-8");
                    ...
                    accname=(rc.getReceipttype()==9||rc.getReceipttype()==2)? customerNames : ( (cust!=null)? cust.getName() : (vendor!=null)? vendor.getName():"");    //To check Customer object is not null (5067)
...
//                }
                ...
                ExportRecordHandler.addHeaderContents(CompanyPDFHeader,document, writer);
                ExportRecordHandler.addHeaderFooter(CompanyPDFFooter,document, writer); 
                ExportRecordHandler.generateReceiptPDFforVHQ(authHandlerDAOObj, kwlCommonTablesDAOObj, EnglishNumberToWordsOjb, 
                        ...
                        config, AccountName,iscontraentryflag,preText,baseUrl,ismanycrdb, pdoMAp,preparedBy );
                    ...
                    if(mode == StaticValues.AUTONUM_DELIVERYORDER){
                        ...
                        HashMap<String, Object> fieldrequestParams = new HashMap();
                        fieldrequestParams.put(Constants.filter_names, Arrays.asList(Constants.companyid, Constants.moduleid, Constants.customcolumn, "customfield"));
                        fieldrequestParams.put(Constants.filter_values, Arrays.asList(com.getCompanyID(), Constants.Acc_Delivery_Order_ModuleId, 0, 1));
                        FieldMap = accAccountDAOobj.getFieldParamsCustomMap(fieldrequestParams, replaceFieldMap, customFieldMap, customDateFieldMap);

                        Map<String, Object> variableMap = new HashMap<String, Object>();
                        KwlReturnObject idcustresult = accountingHandlerDAOobj.getObject(DeliveryOrderCustomData.class.getName(), orderID);
                        if (idcustresult.getEntityList().size() > 0) {
                        ...
                        Map<String, Object> variableMap = new HashMap<String, Object>();
                        KwlReturnObject idcustresult = accountingHandlerDAOobj.getObject(GoodsReceiptOrderCustomData.class.getName(), orderID);
                        if (idcustresult.getEntityList().size() > 0) {
                            GoodsReceiptOrderCustomData jeCustom = (GoodsReceiptOrderCustomData) idcustresult.getEntityList().get(0);
                            AccountingManager.setCustomColumnValues(jeCustom, FieldMap, replaceFieldMap, variableMap);
                            for (Map.Entry<String, Object> varEntry : variableMap.entrySet()) {
                                String coldata = varEntry.getValue().toString();
                                if (!StringUtil.isNullOrEmpty(coldata)) {
                ...
                if (isCompanyTemplateLogo) {
                    tab2 = ExportRecordHandler.getDateTableWithJobIDforVHQ(entryDate, invno, datetheader, formatter, invCell,jobNumber);
