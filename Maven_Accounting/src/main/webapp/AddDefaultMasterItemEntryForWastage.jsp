...
<%
    ...
    try {
        ...
        while (rs1.next()) {
            ...
            query = "select id from masteritem where masterGroup=31 and value='Wastage' and company=? "; // Master Group id for Stock Adjustment is 31
            PreparedStatement stmt2 = conn.prepareStatement(query);
            stmt2.setString(1, companyID);
            ResultSet rs2 = stmt2.executeQuery();
            String masterID = "";
            while (rs2.next()) {
                masterID = rs2.getString("id");
                break;
            }
            if (StringUtil.isNullOrEmpty(masterID)) { // This check avoid duplicate entry when script executed more than one time by mistake 
                String uuid = UUID.randomUUID().toString().replace("-", "");
                query = " insert into masteritem (id,value,masterGroup,company,defaultmasteritem) values (?,?,?,?,?) ";
                PreparedStatement stmt3 = conn.prepareStatement(query);
                stmt3.setString(1, uuid);
                stmt3.setString(2, "Wastage");
                stmt3.setString(3, "31");
                stmt3.setString(4, companyID);
                stmt3.setString(5, Constants.WASTAGE_ID);
                stmt3.execute();
                cnAddCounter++;
                out.println("<br><br> Default Master Item Entry of 'Wastage' for 'Stock Adjustement' is added successfully <b>Added</b> for " + companyName + " : " + cnAddCounter);
