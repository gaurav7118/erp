...
<%
    ...
    try {
        // Customer Invoice
        //SCRIPT URL : http://<app-url>/CustomerInvoiceDescUpdate.jsp?serverip=?&dbname=?&username=?&password=?&subdomain=?
        ...
        if (!StringUtil.isNullOrEmpty(subdomain)) {
            ...
            while (compresult.next()) {
                ...
                querycashInv = "SELECT invd.id, invd.description, invd.invoice, inv.invoicenumber FROM invoicedetails invd INNER JOIN invoice inv ON invd.invoice=inv.id WHERE invd.company='" + companyid + "' AND invd.description IS NOT NULL ORDER BY inv.invoicenumber";
...
<div align="center">
    <table border="1" cellpadding="5">
        ...
        <tr><th>Subdomain</th><th>Invoice Number</th><th>Description</th></tr>
        <%
            while (rs1.next()) {
                id1 = rs1.getString("id");
                desc1 = rs1.getString("description");
                compsubdomain = subdomain;
                txnnumber = rs1.getString("invoicenumber");
                try {
                    decodeDescription("invoicedetails", id1, desc1, conn);
        %>
        <tr>
...
<%
    ...
    while (compresult.next()) {
        ...
        querycashInv = "SELECT invd.id, invd.description, invd.invoice, inv.invoicenumber FROM invoicedetails invd INNER JOIN invoice inv ON invd.invoice=inv.id WHERE invd.description IS NOT NULL ORDER BY inv.invoicenumber";
        ps1 = conn.prepareStatement(querycashInv);
        rs1 = ps1.executeQuery();
%>
<div align="center">
