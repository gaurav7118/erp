...
<%
    ...
    try {
        ...
        if (!StringUtil.isNullOrEmpty(subDomain)) {
            query = "SELECT p.id,p.company,p.journalentry,p.receiptnumber,p.externalcurrencyrate,p.bankchargesamount,p.bankchargesaccountid,p.deposittojedetail,p.depositamount,p.journalentryforbankcharges,p.paydetail,"
                        + "c.subdomain FROM receipt p inner join company c on p.company=c.companyid where c.subdomain = ? and p.bankchargesamount>? ";
//                + "c.subdomain FROM receipt p inner join company c on p.company=c.companyid where c.subdomain = ? and p.receiptnumber='RP101'";
            ...
            query = "SELECT p.id,p.company,p.journalentry,p.receiptnumber,p.externalcurrencyrate,p.bankchargesamount,p.bankchargesaccountid,p.deposittojedetail,p.depositamount,p.journalentryforbankcharges,p.paydetail,"
                    + "c.subdomain FROM receipt p inner join company c on p.company=c.companyid where p.bankchargesamount>? order by c.subdomain";
//                + "c.subdomain FROM receipt p inner join company c on p.company=c.companyid where c.subdomain = ? and p.receiptnumber='RP101'";
            stmtquery = conn.prepareStatement(query);
        ...
        while (rs.next()) {
            ...
            String journalentryforbankcharges = rs.getString("journalentryforbankcharges");
            if (StringUtil.isNullOrEmpty(journalentryforbankcharges)) {

                company = rs.getString("company");
                double bankchargesamount = rs.getDouble("bankchargesamount");
                String bankchargesaccountid = rs.getString("bankchargesaccountid");
                ...
                while (resultSet.next()) {
                    ...
                    // insert JE Details for bank charges debit
                    ...
                    preparedStatement.setDouble(3, bankchargesamount);
                    preparedStatement.setString(4, bankchargesaccountid);
...
//                    
//                }
                    finalamt = amount - bankchargesamount;
                    ...
                    // delete Bank charge detail from Main JE
                    String deletequery = "delete from jedetail where journalEntry=? and account=?";
                    PreparedStatement statement = conn.prepareStatement(deletequery);
                    statement.setString(1, mainJE);
                    statement.setString(2, bankchargesaccountid);
                    statement.executeUpdate();

                    // insert JE Details for bank charges credit
                ...
                // Update Payment
                String updatepayment = "update receipt set depositamount=?,journalentryforbankcharges=? where id=? and  company=?";
                PreparedStatement statement1 = conn.prepareStatement(updatepayment);
                depositamount = depositamount - bankchargesamount;
