...
//        DataInputStream in = new DataInputStream(invoice);
        ...
        String companyId = "fd64e327-606e-400c-b3eb-abe8e7316008";    //companyid for sanxing
        String bbfrom = "";
        ...
        while ((record = in.readLine()) != null) {
            ...
            if (record != "") {
                ...
                String warehouse = recarrName[3].trim();


                if ("Product ID".equals(productCode) && "Warehouse".equals(warehouse)) {
                    continue;
                }


                double qty = Double.parseDouble(recarrName[1].trim());
                String location = recarrName[2].trim();
                String serialName = recarrName[5].trim();
                String batchName = recarrName[4].trim();
                price = Double.parseDouble(recarrName[6].trim());
                ...
                //    }
                if (!productCode.equals(sameProduct)) {
                    String queryForAccId = " update product set purchaseaccount='4028e4d3537c4e6801537ef88bfd57bb',purchasereturnaccount='4028e4d3537c4e6801537ef88bfd57bb',salesaccount='4028e4d3537c4e6801537ef88b8757b5',salesreturnaccount='4028e4d3537c4e6801537ef88b8757b5',location=? where company=? and id=?";

                    PreparedStatement stmta = conn.prepareStatement(queryForAccId);
                    stmta.setObject(1, locationid);
                    stmta.setObject(2, companyId);
                    stmta.setObject(3, productId);
                    stmta.executeUpdate();
                }
                String InvId = "";
                ...
                if (!batchName.equals("-")) {
                    stmtbatch.setObject(5, batchName);
                }
                ...
                if (!StringUtil.isNullOrEmpty(batchId)) {
                    ...
                    String querybatch = "insert into newproductbatch (id,quantity,quantitydue,location,warehouse,transactiontype,product,isopening,ispurchase,company,batchname) values (?,?,?,?,?,28,?,'T','T',?,?)";
                    ...
                    stmtb.setString(8, batchName.equals("-") ? "" : batchName);
                    stmtb.execute();
                    stmtb.close();
