...
//        DataInputStream in = new DataInputStream(invoice);
        ...
        String companyId = "3e5acf3b-91b1-4c7c-88d3-f51a2981c50b";    //companyid for ABS
        String bbfrom = "2015-06-30";
        ...
        while ((record = in.readLine()) != null) {
            ...
            if (record != "") {
                ...
                String warehouse = recarrName[4].trim();
                String location = recarrName[3].trim();
                price = Double.parseDouble(recarrName[2].trim());
                ...
                //    }
                String queryForAccId = "update product set asofdate=? where company=? and id=?";

                PreparedStatement stmta = conn.prepareStatement(queryForAccId);
                stmta.setObject(1, bbfrom);
                ...
                if (!StringUtil.isNullOrEmpty(InvId)) {
                    String updatequery = "update inventory set quantity=?,baseuomquantity=?,carryin='T',defective='F',newinv='T',updatedate=?,deleteflag='F' where id=?";
                ...
                if (rssm.next()) {
                    ...
                    stmtforSMDUpdate.setDouble(1, qty);
