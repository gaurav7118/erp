...
<%
    ...
    try {
        // JE Details Description Update
        //SCRIPT URL : http://<app-url>/CNDNDescUpdate.jsp?serverip=?&dbname=?&username=?&password=?&subdomain=?
        ...
        String id1 = "",  desc1 = "",  entryno = "", compsubdomain = "", companyid = "", compName = "", querycashInv = "";
        String companyQry = "";
        ...
        if (!StringUtil.isNullOrEmpty(subdomain)) {
            ...
            querycashInv = "SELECT jed.id, jed.description, je.entryno from jedetail jed INNER JOIN journalentry je ON jed.journalEntry=je.id WHERE jed.company='" + companyid + "' AND jed.description IS NOT NULL ORDER BY je.entryno";
...
<div align="center">
    <table border="1" cellpadding="5">
        ...
        <tr><th>Subdomain</th><th>JE Number</th><th>Description</th></tr>
        <%
            while (rs1.next()) {    
                ...
                entryno = rs1.getString("entryno");
                try {
                    decodeDescription("jedetail", id1, desc1, conn);
        %>
        <tr>
            <td><% out.println(compsubdomain);%></td>
            <td><% out.println(entryno);%></td>
