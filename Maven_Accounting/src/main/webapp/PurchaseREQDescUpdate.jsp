    ...
    public void decodeDescription(String tablename, String id, String desc, Connection conn) {  //Recursive Method
        ...
        try {
            ...
            if (originalVal.equalsIgnoreCase(decodedVal)) {
                ...
                try {
                    String updateQuery = "UPDATE " + tablename + " SET productdescription=?  where id=?";
...
<%
    ...
    try {
        ...
        //SCRIPT URL : http://<app-url>/PurchaseREQDescUpdate.jsp?serverip=?&dbname=?&username=?&password=?&subdomain=?
        ...
        String id1 = "", desc1 = "", txnnumber = "", compsubdomain = "", companyid = "", compName = "", querycashInv = "";
        String companyQry = "";
        ...
        if (!StringUtil.isNullOrEmpty(subdomain)) {
            ...
            while (compresult.next()) {
                ...
                querycashInv = "SELECT pq.id, pq.productdescription, pr.prnumber from purchaserequisitiondetail pq INNER JOIN purchaserequisition pr ON pq.purchaserequisition=pr.id WHERE pq.company='" + companyid + "' AND pq.productdescription IS NOT NULL ORDER BY pr.prnumber";
...
<div align="center">
    <table border="1" cellpadding="5">
        ...
        <tr><th>Subdomain</th><th>P.Req. Number</th><th>Description</th></tr>
        <%
            while (rs1.next()) {
                id1 = rs1.getString("id");
                desc1 = rs1.getString("productdescription");
                compsubdomain = subdomain;
                txnnumber = rs1.getString("prnumber");
                try {
                    decodeDescription("purchaserequisitiondetail", id1, desc1, conn);
        %>
        <tr>
...
<%
    ...
    while (compresult.next()) {
        ...
        querycashInv = "SELECT pq.id, pq.productdescription, pr.prnumber from purchaserequisitiondetail pq INNER JOIN purchaserequisition pr ON pq.purchaserequisition=pr.id WHERE pq.productdescription IS NOT NULL ORDER BY pr.prnumber";
        ps1 = conn.prepareStatement(querycashInv);
        rs1 = ps1.executeQuery();
%>
<div align="center">
