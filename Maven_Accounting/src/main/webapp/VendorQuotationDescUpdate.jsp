...
<%
    ...
    try {
        // Vendor Quotation
        //SCRIPT URL : http://<app-url>/VendorQuotationDescUpdate.jsp?serverip=?&dbname=?&username=?&password=?&subdomain=?
        ...
        if (!StringUtil.isNullOrEmpty(subdomain)) {
            ...
            while (compresult.next()) {
                ...
                querycashInv = "SELECT vd.id, vd.description, vq.quotationnumber from vendorquotationdetails vd INNER JOIN vendorquotation vq ON vd.vendorquotation=vq.id WHERE vd.company='" + companyid + "' AND vd.description IS NOT NULL ORDER BY vq.quotationnumber";
...
<div align="center">
    <table border="1" cellpadding="5">
        ...
        <tr><th>Subdomain</th><th>VQ Number</th><th>Description</th></tr>
        <%
            while (rs1.next()) {
                id1 = rs1.getString("id");
                desc1 = rs1.getString("description");
                compsubdomain = subdomain;
                txnnumber = rs1.getString("quotationnumber");
                try {
                    decodeDescription("vendorquotationdetails", id1, desc1, conn);
        %>
        <tr>
...
<%
    ...
    while (compresult.next()) {
        ...
        querycashInv = "SELECT vd.id, vd.description, vq.quotationnumber from vendorquotationdetails vd INNER JOIN vendorquotation vq ON vd.vendorquotation=vq.id WHERE vd.description IS NOT NULL ORDER BY vq.quotationnumber";
        ps1 = conn.prepareStatement(querycashInv);
        rs1 = ps1.executeQuery();
%>
<div align="center">
