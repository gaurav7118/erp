...
function setAllAttribs(elm) {
    	var formObj = document.forms[0];
    var href = unescape(formObj.href.value);
    var target ='_self';
    if(formObj.targetlist.value)
        target = getSelectValue(formObj, 'targetlist');
    var temp = href.split('&f=');
    var fname=[];
    var wid=[];
    var onclick_fun;
    var pureHrefFlag=true;
    if(temp[1]){
        fname = temp[1].split('&t=');
        temp = href.split('?i=');
        wid = temp[1].split('&w=');
        if(fname[0]&&temp[0]&&wid[0]){
         href = "javascript:openDownloadWindow_postfix('"+escape(fname[0])+"',true,'"+wid[0]+"','workspacedocs','"+target+"')";
        pureHrefFlag=false;
        }
    }
	...
	setAttrib(elm, 'id');
	setAttrib(elm, 'style');
    var imageclass = getSelectValue(formObj, 'classlist');
    if(imageclass=='' && fname && fname[0] && temp[1]){
        var type=getimage(fname[0]);
        imageclass=(!type)?"padd pwnd "+type:"";
    }
    if(imageclass==""){
        imageclass=tinyMCEPopup.getLang('not_set');
    }
	setAttrib(elm, 'title');
	setAttrib(elm, 'target', target == '_self' ? '' : target);
	setAttrib(elm, 'id');
