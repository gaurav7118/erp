...
Wtf.MRPFileBrowseButton = Wtf.extend(Wtf.Button,{
	input_name : 'file',
	input_file : null,
	...
	initComponent : function() {
		Wtf.MRPFileBrowseButton.superclass.initComponent.call(this);
	...
	onRender : function(ct, position) {
		Wtf.MRPFileBrowseButton.superclass.onRender.call(this, ct, position);
	...
	autoWidth : function() {
		Wtf.MRPFileBrowseButton.superclass.autoWidth.call(this);
		this.adjustInputFileBox();
	},
	...
	disable : function() {
		Wtf.MRPFileBrowseButton.superclass.disable.call(this);
		this.input_file.dom.disabled = true;
	},

	enable : function() {
		Wtf.MRPFileBrowseButton.superclass.enable.call(this);
	...
	destroy : function() {
		...
		input_file.remove();
		input_file = null;
		Wtf.MRPFileBrowseButton.superclass.destroy.call(this);
	},
...
Wtf.MRPMultiFlieUploadPanel = Wtf.extend(Wtf.Panel, {
	autoUpload:false,
	initComponent:function(){
	    ...
	    var store = new Wtf.data.Store({
            ...
            if(this.fileStr !=undefined && this.fileStr != ""){
                Wtf.Ajax.request({
	        url : "ACCContractMaster/getTemporarySavedFiles.do",
	        params : {fileid:this.fileStr},
	...
	updateRecordState:function(record, state, message,fileid){
	...
	onAjaxSuccess:function(resp, data){
	    ...
	    if (result.data.success==true) {
	    	...
	    	msg = WtfGlobal.getLocaleText("acc.field.UplodedSuccessfully");
                    ...
                    this.url='ACCContractMasterCMN/attachDocuments.do?type=doc&savedFilesMappingId='+this.savedFilesMappingId;
                    this.savedFilesId=fileid;
                    this.fireEvent("uploadComplete",this);
	    }else{
	    	msg=result.data.msg||WtfGlobal.getLocaleText("acc.field.UploadFailed");
	    }
	    this.updateRecordState(data.record, state, msg,fileid);
		if(this.getQueuedCount() > 0){
			this.prepareNextUploadTask();
		}else{
			this.uploading = false;
