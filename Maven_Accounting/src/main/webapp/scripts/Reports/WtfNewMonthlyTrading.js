...
function monthlyRevenueDynamicLoad(){
    var panel = Wtf.getCmp("monthlyRevenue");
    if(panel==null){
        panel = new Wtf.account.MonthlyTradingAndProfitLoss({
            id : 'monthlyRevenue',
            ...
            topTitle:'<center><font size=4>' + WtfGlobal.getLocaleText("acc.monthlyRevenue.tabTitle") + '</font></center>',
...
function monthlyTradingProfitLossDynamicLoad(params){
    var panel = Wtf.getCmp("newMonthlyTradingProfitandLoss");
    ...
    if (params.isCustomWidgetReport) {
         ...
         * */
        panel = new Wtf.account.MonthlyTradingAndProfitLoss({
            tabTip: WtfGlobal.getLocaleText("acc.MonthlyP&L.tabTitleTT"),
            topTitle: '<center><font size=4>' + WtfGlobal.getLocaleText("acc.MonthlyP&L.tabTitle") + '</font></center>',
            statementType: 'MonthlyTradingAndProfitLoss',
            border: false,
            layout: 'fit',
    ...
    if(panel==null){
        panel = new Wtf.account.MonthlyTradingAndProfitLoss({
            id : 'newMonthlyTradingProfitandLoss',
            title:Wtf.util.Format.ellipsis(WtfGlobal.getLocaleText("acc.MonthlyP&L.tabTitle"),Wtf.TAB_TITLE_LENGTH),
            tabTip:WtfGlobal.getLocaleText("acc.MonthlyP&L.tabTitleTT"),
            topTitle:'<center><font size=4>' + WtfGlobal.getLocaleText("acc.MonthlyP&L.tabTitle") + '</font></center>',
            statementType:'MonthlyTradingAndProfitLoss',
            border : false,
...
//**********************************************************************************
Wtf.account.MonthlyTradingAndProfitLoss=function(config){
    this.total=[0,0];
    ...
    this.isCustomWidgetReport=config.isCustomWidgetReport||false; //Flag for widget view.
...
//    this.rStroe.on('load', this.hideLoading, this);
//    this.rStroe.on('loadexception', this.hideLoading, this);
...
//    this.rGrid.on('cellclick',this.onCellClick, this);
    ...
    this.wrapperPanel = new Wtf.Panel({
        border:false,
        layout:"border",
        title:WtfGlobal.getLocaleText("acc.MonthlyP&L.MonthlyP&LReportHelpMsg"),
        scope:this,
    ...
    Wtf.apply(this,{
     ...
     Wtf.account.MonthlyTradingAndProfitLoss.superclass.constructor.call(this,config);
     this.addEvents({
        'account':true
     });
...
Wtf.extend( Wtf.account.MonthlyTradingAndProfitLoss,Wtf.Panel,{
...
//        if(el==null)return;
    ...
    exportPdfTemplate:function(){
        ...
        if(this.statementType=="BalanceSheet"){
            ...
            extraParams +="&searchJson="+(this.searchJson!=undefined?this.searchJson:"")+"&filterConjuctionCriteria="+this.filterConjuctionCrit;
    ...
    showChart : function(param) {
        var chartPanelID = "MonthlyTradingProfitLossLineChart";
        var chartParams = this.getLineChartParams();
        chartParams.id = chartPanelID;
        chartParams.title = WtfGlobal.getLocaleText("acc.common.MonthlyTradingPnL.LineChart");
        chartParams.url = this.getLineChartUrl();
        chartParams.chartConfig = this.getLineChartConfig(chartParams);
        chartParams.tabTipParams = [WtfGlobal.getLocaleText("acc.common.TT.MonthlyTradingPnL")];
    ...
    getStoreBaseParams: function () {
        ...
        currentBaseParams.searchJson = this.searchJson != undefined ? this.searchJson : "";
        currentBaseParams.filterConjuctionCriteria = this.filterConjuctionCrit
    ...
    getLineChartUrl: function () {
        return "ACCReports/getMonthlyTradingAndProfitLoss.do";
    },
...
//        var lineColorArr = ["#A25FAC", "#FF0000", "#00D1FF", "#FFFF00", "#FF00FF", "#0066CB", "#993233"];
        var lineColorArr = ["#A25FAC", "#FF0000", "#00D1FF", "#0066CB", "#993233"];
...
//        params.textColor = "#009987";
        ...
        params.title = "Monthly Trading Profit and Loss Report";
    ...
    exportWithTemplate:function(type){
        ...
        var fileName;
        var reportName;
        var header = "accountname,accountcode";
        var isDimMonthlyPL=false;
...
//            }
            if(isDimMonthlyPL){
                exportUrl = getExportUrl(Wtf.autoNum.DimensionBasedMonthlyTradingPL);
            }else{
                exportUrl = getExportUrl(Wtf.autoNum.MonthlyTradingPnl);
            }
            ...
            if(this.statementType=="MonthlyTradingAndProfitLoss"){
                if(isDimMonthlyPL){
                    fileName =  WtfGlobal.getLocaleText("acc.report.DimBasedMonthlyPLfileName")+"_v1";
...
//                    +"&header="+header+"&title="+encodeURIComponent(title)+"&name="+encodeURIComponent(reportName)+"&width=150&get=27&align="+align+"&toggle="+this.toggle+"&singleGrid="+true+"&dimensionBasedMonthlyPL="+isDimMonthlyPL+"&isMonthlyReport="+true;

        var urlExcludingParameter = exportUrl + "?";
        var parametersExcludingUrl = "filename=" + encodeURIComponent(fileName) +
                                    extraParams + "&filetype=" + type
                                    + "&stdate=" + this.sDate + "&enddate=" + this.eDate + "&nondeleted=" + true + "&accountid="
                                    + "&header=" + header + "&title=" + encodeURIComponent(title) + "&name=" + encodeURIComponent(reportName) + "&width=150&get=27&align=" + align + "&toggle=" + this.toggle + "&singleGrid=" + true + "&dimensionBasedMonthlyPL=" + isDimMonthlyPL + "&isMonthlyReport=" + true;

        var url = urlExcludingParameter + parametersExcludingUrl;
        if (type == "print") {
...
//             Wtf.get('downloadframe').dom.src  = url;
            WtfGlobal.postData(urlExcludingParameter, parametersExcludingUrl);
        }
//        Wtf.get('downloadframe').dom.src = url;
    },
