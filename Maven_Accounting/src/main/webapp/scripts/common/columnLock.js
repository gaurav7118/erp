...
Wtf.grid.LockCheckboxSelectionModel = Wtf.extend(Wtf.grid.CheckboxSelectionModel, {
    isLocked: Wtf.emptyFn,
    initEvents : function(){
        Wtf.grid.LockCheckboxSelectionModel.superclass.initEvents.call(this);
        this.grid.on('render', function(){
            var view = this.grid.getView();
            view.mainBody.on('mousedown', this.onMouseDown, this);
            Wtf.fly(view.lockedInnerHd).on('mousedown', this.onHdMouseDown, this);
        }, this);
    },
...
Wtf.grid.LockingGridPanel = Wtf.extend(Wtf.grid.GridPanel,{
    getView : function(){
        if(!this.view){
            this.view = new Wtf.grid.LockingGridView(this.viewConfig);
        }
        return this.view;
    },
    ...
    initComponent : function(){
        if(!this.cm) {
            this.cm = new Wtf.grid.LockingColumnModel(this.columns);
            delete this.columns;
        }
        Wtf.grid.LockingGridPanel.superclass.initComponent.call(this);
    },
    getState : function(){
        var o = {
            columns: []
    ...
    initStateEvents : function(){
        Wtf.grid.LockingGridPanel.superclass.initStateEvents.call(this);
        this.colModel.on('columnlockchange', this.saveState, this, {delay: 100});
    }
});

Wtf.grid.LockingEditorGridPanel = Wtf.extend(Wtf.grid.EditorGridPanel,{
    getView : function(){
        if(!this.view){
            this.view = new Wtf.grid.LockingGridView(this.viewConfig);
        }
    ...
    initComponent : function(){
        ...
        Wtf.grid.LockingEditorGridPanel.superclass.initComponent.call(this);
    }
});

Wtf.grid.LockingGridView = Wtf.extend(Wtf.grid.GridView, {
    ...
    initTemplates : function(){
        ...
        if(!this.templates.master){
            this.templates.master = new Wtf.Template(
                ...
                '</div>',
                '<div class="x-grid3-viewport" style = "position:absolute;overflow:hidden;">',
        ...
        Wtf.grid.LockingGridView.superclass.initTemplates.call(this);
    },
    ...
    scrollToTop : function(){
        Wtf.grid.LockingGridView.superclass.scrollToTop.call(this);
        this.syncScroll();
    },

    syncScroll : function(e){
        Wtf.grid.LockingGridView.superclass.syncScroll.call(this, e);
...
//            row.style.height = lrow.style.height = h + "px";
    ...
    insertRows : function(dm, firstRow, lastRow, isUpdate){
        if(firstRow === 0 && lastRow == dm.getCount()-1){
            ...
            if(before){
                Wtf.DomHelper.insertHtml('beforeBegin', before, html[0]);
            } else {
                Wtf.DomHelper.insertHtml('beforeEnd', this.mainBody.dom, html[0]);
            }
            var beforeLocked = this.getLockedRow(firstRow);
            if (beforeLocked) {
                Wtf.DomHelper.insertHtml('beforeBegin', beforeLocked, html[1]);
            }else{
                Wtf.DomHelper.insertHtml('beforeEnd', this.lockedBody.dom, html[1]);
            }
            if(!isUpdate){
    ...
    removeRow : function(row) {
        ...
        if (this.cm.getLockedCount() > 0) {
            Wtf.removeNode(this.getLockedRow(row));
        }
    },
    ...
    renderUI : function(){
        ...
        Wtf.fly(this.lockedInnerHd).on("click", this.handleHdDown, this);
        ...
        if(g.enableColumnResize !== false){
            this.splitone = new Wtf.grid.GridView.SplitDragZone(g, this.lockedHd.dom);
            this.splitone.setOuterHandleElId(Wtf.id(this.lockedHd.dom));
            this.splitone.setOuterHandleElId(Wtf.id(this.mainHd.dom));
        }
    ...
    refreshRow : function(record){
        Wtf.grid.LockingGridView.superclass.refreshRow.call(this, record);
        var index = this.ds.indexOf(record);
        this.getLockedRow(index).rowIndex = index;
    }
});

Wtf.grid.LockingColumnModel = Wtf.extend(Wtf.grid.ColumnModel,{
