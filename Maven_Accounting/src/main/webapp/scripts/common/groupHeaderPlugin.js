Wtf.GroupHeaderGrid = function(config) {
	this.config = config;
};

Wtf.extend(Wtf.GroupHeaderGrid, Wtf.util.Observable, {
	init: function(grid) {
		Wtf.applyIf(grid.colModel, this.config);
		Wtf.apply(grid.getView(), this.viewConfig);
	},
	...
	viewConfig: {
		...
		renderHeaders: function() {
			...
			for (var row = 0, rlen = rows.length; row < rlen; row++) {
				...
				for (var i = 0, gcol = 0, len = r.length; i < len; i++) {
					...
					var gs = Wtf.GroupHeaderGrid.prototype.getGroupStyle.call(this, group, gcol);
					cells[i] = ts.gcell.apply({
						...
						tooltip: group.tooltip ? (Wtf.QuickTips.isEnabled() ? 'Wtf:qtip' : 'title') + '="' + group.tooltip + '"' : '',
		...
		onColumnWidthUpdated : function(){
			...
			Wtf.GroupHeaderGrid.prototype.updateGroupStyles.call(this);
		},
		...
		beforeColMenuShow : function(){
			...
			for(var col = 0, clen = cm.getColumnCount(); col < clen; col++){
				...
				if(cm.config[col].fixed !== true && cm.config[col].hideable !== false){
					for (var row = 0, rlen = rows.length; row < rlen; row++) {
						...
						if (group && group.header) {
							if (cm.hierarchicalColMenu) {
								...
								if (!submenu) {
									submenu = new Wtf.menu.Menu({id: gid});
					...
					menu.add(new Wtf.menu.CheckItem({
		...
		renderUI : function(){
			...
			Wtf.apply(this.columnDrop, Wtf.GroupHeaderGrid.prototype.columnDropConfig);
		}
	},
	...
	columnDropConfig : {
		...
		positionIndicator : function(h, n, e){
			var data = Wtf.GroupHeaderGrid.prototype.getDragDropData.call(this, h, n, e);
			if (data === false) {
				return false;
			}
		...
		onNodeDrop : function(n, dd, e, data){
			var h = data.header;
			if(h != n){
				var d = Wtf.GroupHeaderGrid.prototype.getDragDropData.call(this, h, n, e);
				if (d === false) {
					return false;
				}
	...
	getGroupStyle: function(group, gcol) {
		...
		return {
			width: (Wtf.isBorderBox ? width : Math.max(width - this.borderWidth, 0)) + 'px',
			hidden: hidden
		};
	},
    ...
    getDragDropData : function(h, n, e){
		...
		var row = Wtf.GroupHeaderGrid.prototype.getGroupRowIndex.call(this.view, h);
		var oldGroup = Wtf.GroupHeaderGrid.prototype.getGroupSpan.call(this.view, row, oldIndex);
		var newGroup = Wtf.GroupHeaderGrid.prototype.getGroupSpan.call(this.view, row, newIndex);
		...
		var parentGroup = Wtf.GroupHeaderGrid.prototype.getGroupSpan.call(this.view, row - 1, oldIndex);
