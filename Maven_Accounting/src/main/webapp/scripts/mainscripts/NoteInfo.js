...
Wtf.account.NoteInfoWindow = function(config) {
    ...
    this.isVendorOnly = config.isVendorOnly;
    this.personInfo=config.personInfo;
    this.butnArr = [];
    this.title = this.isCN ? WtfGlobal.getLocaleText("acc.module.name.12") : WtfGlobal.getLocaleText("acc.module.name.10");
    this.isEdit=config.isEdit;
    this.billid=config.billid;
    this.isMulticurrency=config.isMulticurrency;
    this.moduleid = this.isReceipt ? Wtf.Acc_Debit_Note_ModuleId :Wtf.Acc_Credit_Note_ModuleId;
    this.quickPanelSearch = new Wtf.KWLTagSearch({
        emptyText:WtfGlobal.getLocaleText("acc.field.SearchbyNoteNo"),  //'Search by Note No.',
        width: 200,
    ...
    Wtf.account.NoteInfoWindow.superclass.constructor.call(this, config);
}
Wtf.extend(Wtf.account.NoteInfoWindow, Wtf.Window, {
    height: 480,
    width: 800,
    modal: true,
    iconCls : 'pwnd deskeralogoposition',
    onRender: function(config) {
        Wtf.account.NoteInfoWindow.superclass.onRender.call(this, config);
        this.createDisplayGrid();        
        var msg = this.isCustomer ? "<b>"+WtfGlobal.getLocaleText("acc.contractDetails.CustomerName")+"</b> : " + this.personInfo.personName + "<br> <b>"+WtfGlobal.getLocaleText("acc.customer.customercode")+"</b> : " + this.personInfo.personCode : "<b>"+WtfGlobal.getLocaleText("acc.ven.name")+"</b> : " + this.personInfo.personName + "<br> <b>"+WtfGlobal.getLocaleText("acc.vendor.vendorcode")+"</b> : " + this.personInfo.personCode ;
        msg=msg+'<div style="font-size:14px; text-align:left; font-weight:bold; margin-top:1%;">'+WtfGlobal.getLocaleText("acc.field.Note")+': </div>'+'<div style="font-size:12px; text-align:left; margin-top:1%;">'+WtfGlobal.getLocaleText("acc.makepayment.documenttype.note")+'</div>'
        var isgrid = true;
        this.add({
            region: 'north',
            height: 115,
...
//                displayMsg: 'Displaying records {0} - {1} of {2}',
    ...
    createDisplayGrid: function() {
        ...
        this.cm = new Wtf.grid.ColumnModel([this.sm, {
                header: this.isCN ? WtfGlobal.getLocaleText("acc.cnList.gridNoteNo") : WtfGlobal.getLocaleText("acc.dnList.gridNoteNo"), //"CN Number"/"DN Number"
                dataIndex: 'documentno'
            }, {
                header: this.isCN ? WtfGlobal.getLocaleText("acc.cnList.gridDate") : WtfGlobal.getLocaleText("acc.dnList.gridDate"), //"CN Date"/"DN Date"
                dataIndex: 'date',
                align:'center',
        ...
        this.Rec = new Wtf.data.Record.create([
            {name: 'documentno', mapping: 'noteno'},
            {name: 'date',type: 'date'},
            ...
            {name: 'currencysymbol'},
            {name: 'currencyid'},
            {name:'documentid' , mapping: 'noteid' },
            {name: 'accountid'},
            {name:'accountnames'},
        ...
        this.store = new Wtf.data.Store({
            ...
            url: this.isCN ? "ACCCreditNote/getCreditNoteMergedForPayment.do" : "ACCDebitNote/getDebitNoteMergedForPayment.do",
            baseParams: {
                ...
                billId:this.billid,
                isEdit:this.isEdit,
                isNoteForPayment:this.isMulticurrency ? false : true,
        ...
        var NoteStoreParams = {ss: this.quickPanelSearch.getValue(),
                                  start:0,
                                  limit:30}
//       if(this.isMulticurrency){                // 'isNOteForPayment' flag is used at back-end to decide whether all notes to be returned or only notes with payment currency to be returned'
//           NoteStoreParams['isNoteForPayment']=false;
//       } else {
//           NoteStoreParams['isNoteForPayment']=true;
//       }                      
        this.store.load({params: NoteStoreParams});
...
//            border: false,
    ...
    submitSelectedRecords: function() {
        ...
        if (selections.length == 0) {
            WtfComMsgBox([WtfGlobal.getLocaleText("acc.common.warning"), this.isCN ? WtfGlobal.getLocaleText("acc.alert.nocnselected") : WtfGlobal.getLocaleText("acc.alert.nodnselected")], 2);// 'No CN/DN selected. Select CN/DN first'
            return;
        }
        this.close();
    },
