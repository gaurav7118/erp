 ...
 */
Wtf.account.WriteOffPaymentsWindow = function(config) {
    ...
    this.writeOffRecords = new Wtf.Toolbar.Button({
        text:this.isReceipt?WtfGlobal.getLocaleText("acc.writeOff.writeOffReceipts"):WtfGlobal.getLocaleText("acc.writeOff.writeOffPayments"),  
        tooltip:this.isReceipt?WtfGlobal.getLocaleText("acc.writeOff.writeOffReceipts"):WtfGlobal.getLocaleText("acc.writeOff.writeOffPayments"),
        style:"margin-left: 15px;",
        iconCls:'writeOffSelectedSalesInvoice',
        scope:this,
        disabled:true,
        handler:this.writeOffselectedRecords                        
    });
    this.butnArr.push({
    ...
    Wtf.account.WriteOffPaymentsWindow.superclass.constructor.call(this, config);
}
Wtf.extend(Wtf.account.WriteOffPaymentsWindow, Wtf.Window, {
    height: 490,
    width: 800,
    modal: true,
    iconCls : 'writeOffSalesInvoice',
    onRender: function(config) {
        Wtf.account.WriteOffPaymentsWindow.superclass.onRender.call(this, config);
        this.createDisplayGrid();
        var msg = '';
        msg=msg+ this.isReceipt ? '<b>'+WtfGlobal.getLocaleText("acc.writeOff.receipts")+'</b>':'<b>'+WtfGlobal.getLocaleText("acc.writeOff.payments")+'</b>';
        var isgrid = true;
        this.add({
            region: 'north',
            height: 115,
            ...
            layout: 'fit',
            items: [this.grid],
            tbar:[this.quickPanelSearch,this.resetBttn,WtfGlobal.getLocaleText("acc.common.from"),this.startDate,WtfGlobal.getLocaleText("acc.common.to"),this.endDate,this.fetchBttn,this.writeOffRecords],
    ...
    createDisplayGrid: function() {
        ...
        this.store = new Wtf.data.Store({
            ...
            url: this.isReceipt ? "ACCReceiptCMN/getReceiptsForWriteOff.do" : "ACCVendorPaymentCMN/getPaymentsForWriteOff.do",
            baseParams: {
                deleted: false,
                nondeleted: true,
    ...
    storeLoaded : function() {
        this.writeOffRecords.disable();
        this.quickPanelSearch.StorageChanged(this.store);
    },
    writeOffselectedRecords: function(){
    ...
    finalSave : function(){
        ...
        this.remarkWin = new Wtf.Window({
            ...
            buttons: [{
                ...
                scope: this,
                handler: function() {
                    var ajaxUrl = this.isReceipt ? "ACCWriteOff/writeOffReceipts.do" : "ACCWriteOff/writreOffPayments.do";
                    Wtf.Ajax.requestEx({
                        url: ajaxUrl,
                        params:{
    ...
    onSelection: function(sm,rowIndex,record){
        if(sm.getSelections().length == 0){
            ...
            this.writeOffRecords.enable();
        }
    },
    checkForValidRecords:function(){
        var selectionArray=this.grid.getSelectionModel().getSelections();
        var invalidReceipts='';
        var invalidCount=0;
        if(selectionArray.length>0){
            for(var x=0;x<selectionArray.length;x++){
                var rec = selectionArray[x];
                if(!rec.data.allowwriteoff){
                    invalidCount++;
                    invalidReceipts+=rec.data.billno+",";
                }
            }
        }
        if(invalidCount!=0 && invalidReceipts!=''){
            invalidReceipts = invalidReceipts.substring(0,invalidReceipts.length-1);
            WtfComMsgBox([WtfGlobal.getLocaleText("acc.common.alert"),this.isReceipt?WtfGlobal.getLocaleText("acc.writeOffReceipt.invalidReceiptsAlert"):WtfGlobal.getLocaleText("acc.writeOffReceipt.invalidPaymentsAlert")+'<br>'+'<b>'+invalidReceipts+'<b>'],2);
            return;
        } else {
            this.checkForalredyWrittenOfRecords();
        }
    },
    checkForalredyWrittenOfRecords:function(){
        var selectionArray=this.grid.getSelectionModel().getSelections();
        var invalidReceipts='';
        var invalidCount=0;
        if(selectionArray.length>0){
        ...
        if(invalidCount!=0 && invalidReceipts!=''){
            invalidReceipts = invalidReceipts.substring(0,invalidReceipts.length-1);
            WtfComMsgBox([WtfGlobal.getLocaleText("acc.common.alert"),(this.isReceipt?WtfGlobal.getLocaleText("acc.writeoff.alreadyWrittenOffReceipts"):WtfGlobal.getLocaleText("acc.writeoff.alreadyWrittenOffPayments"))+'<br>'+'<b>'+invalidReceipts+'<b>'],2);
            return;
        } else {
            Wtf.MessageBox.show({
                    title:WtfGlobal.getLocaleText("acc.common.confirm"),
                    msg:this.isReceipt?WtfGlobal.getLocaleText("acc.writeOff.SureToWriteOffReceipts"):WtfGlobal.getLocaleText("acc.writeOff.SureToWriteOffPayments"),
                    width:500,
    ...
    onWriteOffDateChange:function(field,val,oldVal){
        if(val==''){      // Write off date is mandatory
            ...
            // Write off date can not be older than invoice date of any of the selected invoices.
            var selectedRecords = [];
            selectedRecords = this.grid.getSelectionModel().getSelections();
            for(var x=0;x<selectedRecords.length;x++){
                rec = selectedRecords[x];
                receiptDate = rec.data.date;
                receiptDate = receiptDate.setHours(0, 0, 0, 0);
                if(val<receiptDate){
                    WtfComMsgBox([WtfGlobal.getLocaleText("acc.common.warning"), WtfGlobal.getLocaleText("acc.writeOffReceipt.writeOffDateNotValid")], 2);
                    field.setValue(oldVal);
                    break;
                }
            }
