...
<%
    ...
    try {
        ...
        while (rs.next()) {
            ...
            /*
             * as discussed with paritosh sir,need to move stock in ADMIRAX
             * store instead of N/A ,so updating that in following query
             */

            String defaultStoreUpdateQry = "UPDATE  in_storemaster SET isdefault=(if(id=?,1,0)) WHERE company=? ";
            PreparedStatement stmtfords = conn.prepareStatement(defaultStoreUpdateQry);
            stmtfords.setString(1, "c0ea4a9a-fad1-46d6-82e1-69fffa012731");  // ADMIRAX store id for sustenir
            stmtfords.setString(2, companyId);
            stmtfords.executeUpdate();
