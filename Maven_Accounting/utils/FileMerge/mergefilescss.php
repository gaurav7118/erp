<?php
   ...
   define('ARCHIVE_FOLDER', 'cs/archive'); // location to store archive, don't add starting or trailing slashes
      ...
      /*if (CREATE_ARCHIVE && file_exists("$sDocRoot/".ARCHIVE_FOLDER."/$iETag.cache")) {
         ...
         if (CREATE_ARCHIVE) {
            if (true) { // check for valid etag, we don't want invalid requests to fill up archive folder
				if($filepath == "css1.txt")
			               $oFile = fopen("$sDocRoot/style/acc.css", 'w');
				else
				{
					if($filepath == "css-accounting.txt")
						$oFile = fopen("$sDocRoot/style/accounting.css", 'w');
					else
						$oFile = fopen("$sDocRoot/style/accounting-ex.css", 'w');
				}
