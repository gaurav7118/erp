 ...
 * @author krawler
 */
public class IRASGSTForm5SubmissionOperation extends IRASIntegrationUtil implements IntegrationOperation {
    ...
    public JSONObject executeOperation(JSONObject requestJobj, JSONObject integrationAccountDetails) throws JSONException, ServiceException, AccountingException {
        ...
        try{
            ...
            restServiceUrl+=IntegrationConstants.iras_GSTForm5Submission_context;
            Map<String, String> headersMap = createHeadersMap(integrationAccountDetails);
            IRASTokenGenerationOperation irasTokenGenerationOpration=new IRASTokenGenerationOperation();
            JSONObject result=irasTokenGenerationOpration.executeOperation(requestJobj, integrationAccountDetails);
            JSONObject tokenResult=new JSONObject(result.getString("data"));
            headersMap.put("access_token", tokenResult.optString("token"));
            Map<String, Object> otherReqPropertiesMap = createOtherReqPropertiesMap();
            
            JSONObject resJobj = postRequestToRestService(restServiceUrl, Constants.POST, requestJobj.optJSONObject("payload"), IntegrationConstants.integrationPartyId_IRAS, headersMap, otherReqPropertiesMap);
